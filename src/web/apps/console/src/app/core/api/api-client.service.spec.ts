...
describe('ApiClientService', () => {
  ...
  it('should fetch vehicles with query parameters', () => {
    ...
    const req = httpTesting.expectOne(
      (r) => r.url === '/api/vehicles' && r.params.get('status') === 'Active',
    );
    expect(req.request.method).toBe('GET');
    req.flush({
      items: [{ id: '1', rego: 'ABC123', status: 'Active' }],
      totalCount: 1,
      page: 1,
      pageSize: 50,
      totalPages: 1,
  ...
  it('should fetch job tasks with date range filters', () => {
    service
      .getJobTasks({ from: '2026-08-01T00:00:00Z', to: '2026-08-31T23:59:59Z' })
      .subscribe((res) => {
        expect(res.items).toHaveLength(1);
      });

    const req = httpTesting.expectOne(
      (r) => r.url === '/api/dispatch/tasks' && r.params.has('from'),
    );
    expect(req.request.method).toBe('GET');
    req.flush({
      items: [{ id: 't1', title: 'Delivery Job', status: 'Completed' }],
      totalCount: 1,
      page: 1,
      pageSize: 20,
      totalPages: 1,
