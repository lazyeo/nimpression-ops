...
public sealed class AcceptFineCommandHandler(
    IFineRepository fineRepository,
    IUnitOfWork unitOfWork,
    ICurrentUser currentUser,
    IDateTimeProvider? dateTimeProvider = null) : IRequestHandler<AcceptFineCommand, Result>
{
    public async Task<Result> Handle(AcceptFineCommand request, CancellationToken cancellationToken)
    {
        if (currentUser.Role is not (UserRole.Admin or UserRole.Dispatcher))
        {
            return Error.Forbidden("forbidden", "Only administrators or dispatchers can accept fines.");
        }
        ...
        try
        {
            fine.Accept(currentUser.UserId.Value, now, request.ReviewNote);
        }
