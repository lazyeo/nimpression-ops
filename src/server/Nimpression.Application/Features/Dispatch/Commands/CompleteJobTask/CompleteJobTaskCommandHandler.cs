...
/// <summary>
/// 完成派发任务命令处理器（F5.2 / F5.3）。
/// </summary>
public sealed class CompleteJobTaskCommandHandler(
    IJobTaskRepository jobTaskRepository,
    IUnitOfWork unitOfWork,
    ICurrentUser currentUser,
    IDateTimeProvider dateTimeProvider) : IRequestHandler<CompleteJobTaskCommand, Result>
{
    public async Task<Result> Handle(CompleteJobTaskCommand request, CancellationToken cancellationToken)
    {
        ...
        if (currentUser.Role == UserRole.Driver && currentUser.UserId.HasValue)
        {
            ...
            {
                return Error.Forbidden("forbidden", "You cannot complete a task assigned to another driver.");
            }
        }
        ...
        {
            task.Complete(
                request.CompletedAt ?? dateTimeProvider.UtcNow,
                request.ActualDistanceKm.HasValue ? new Kilometres(request.ActualDistanceKm.Value) : null,
                request.EndOdometerKm.HasValue ? new Kilometres(request.EndOdometerKm.Value) : null);
        }
        ...
        {
            return Error.Unprocessable("invalid_completion_data", ex.Message);
        }
